...
public class Main {
    ...
    private ConcurrentLinkedQueue<File> sharedQueue = new ConcurrentLinkedQueue<>();
    ...
    public static void main(String[] args) {
        ...
        for (Directory dir : dirs) {
            ...
            Thread thread = new Thread(() -> {
                ...
                dir.accept(visitorThreadLocal, system.done, system.sharedQueue);

                // After crawling, add identified files to the shared queue
                visitorThreadLocal.get().getFiles().forEach(file -> {
                    system.sharedQueue.add(file);
                    System.out.println("Thread " + Thread.currentThread().getId() +
                            " added file: " + file.getName() + " to shared queue");
                });

                visitorThreadLocal.remove(); // Clean up the thread local storage
        ...
        // Collect identified files from the shared queue
        System.out.println("Main thread collecting identified files from the shared queue:");
        system.sharedQueue.forEach(file -> System.out.println("Main thread collected file: " + file.getName()));
